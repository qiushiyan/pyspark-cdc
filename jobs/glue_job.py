...
from pyspark.sql.functions import col, when, udf, pandas_udf, dense_rank
...
# install latest version of pyarrow
from setuptools.command import easy_install
install_path = "GLUE_LIB"
...
args = getResolvedOptions(
    sys.argv, ['s3_src_bucket', 's3_src_bucket_key'])

bucket = args['s3_src_bucket']
key = args['s3_src_bucket_key']

spark = SparkSession.builder.appName("CDC").getOrCreate()
src_path = f"s3a://{bucket}/{key}"
output_path = f"s3a://pyspark-cdc-output/output"
...
if "LOAD" in key:
    # when DMS replicate main data, pass it to output bucket
    main_data = spark.read.csv(
        src_path, schema="`_c0` INT, `_c1` STRING, `_c2` STRING, `_c3` DATE")
    main_data = (main_data
                 .withColumnRenamed("_c0", "user_id")
                 .withColumnRenamed("_c1", "name")
                 .withColumnRenamed("_c2", "city")
                 .withColumnRenamed("_c3", "last_login"))
    main_data.write.mode("overwrite").csv(output_path)
else:
    cdc_data = spark.read.csv(
        src_path, schema="`_c0` STRING, `_c1` INT, `_c2` STRING, `_c3` STRING, `_c4` DATE")
    main_data = spark.read.csv(
        output_path, schema="`_c0` INT, `_c1` STRING, `_c2` STRING, `_c3` DATE")
    cdc_data = (cdc_data
    ...
    # update main data with cdc data
    for row in cdc_data.collect():
        action = row["action"]
        if action == "D":
            main_data = delete_(main_data, user_id=row["user_id"])
        elif action == "I":
            main_data = insert_(main_data,
                                user_id=row["user_id"],
                                name=row["name"],
                                city=row["city"],
                                last_login=row["last_login"])
        elif action == "U":
            main_data = update_(main_data,
                                user_id=row["user_id"],
                                name=row["name"],
                                city=row["city"],
                                last_login=row["last_login"])
    # prepare main_data for analytics (e.g., regional churn pattern)
    city_window = Window().partitionBy("city")
    prepared_main_data = (main_data
                          .withColumn(
                              "city", when(col("city") == "Nashville-Davidson",
                                           "Nashville").otherwise(col("city"))
                          )
                          .withColumn(
                              "state", city_to_state_dict.get(col("city"))
                          )
                          .withColumn(
    ...
    prepared_main_data.write.csv(output_path)
